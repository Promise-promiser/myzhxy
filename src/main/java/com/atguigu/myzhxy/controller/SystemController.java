...
public class SystemController {
    ...
    **/
    ...
    public Result headerImgUpload(
        ...
        String portraitPath="E:/picture/myzhxy/".concat(newFileName);
        try {
            multipartFile.transferTo(new File(portraitPath));//将上传文件写到服务器上指定的文件
        } catch (IOException e) {
            e.printStackTrace();
        }
        // 响应图片的路径
    ...
    public Result login(
        ...
        System.out.println(sessionVerifiCode);
        //表单中输入的验证码
        ...
        switch (loginForm.getUserType()){
            case 1:
                try {
                    ...
                    if (admin != null) {
                        //用户的类型和用户id转换成一个密文,以token的名称向客户端反馈(生成token字符串)
